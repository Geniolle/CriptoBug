...
import { TOP_30_ASSET_UNIVERSE } from '@/lib/top-assets-universe';
...
const MAX_PAIRS = Number.parseInt(process.env.EXPO_PUBLIC_TOP_ASSETS_MAX_PAIRS ?? '3500', 10);
const TOP_ASSETS_EXCHANGES: SupportedExchange[] = SUPPORTED_EXCHANGES.filter(
...
const GUARANTEE_SAFETY_BUFFER_PERCENT = Number.parseFloat(
  process.env.EXPO_PUBLIC_TOP_ASSETS_GUARANTEE_BUFFER_PERCENT ?? '0.35'
);
const GUARANTEE_MIN_COVERAGE = Number.parseInt(process.env.EXPO_PUBLIC_TOP_ASSETS_GUARANTEE_MIN_COVERAGE ?? '3', 10);
...
function estimateNetProfitPercent(
...
function rankAssetFromCandidates(
  ...
  if (!Number.isFinite(bestNet)) {
    ...
    const fallbackCosts =
      TAKER_FEE_PERCENT[only.exchange] * 2 +
      estimateSlippagePercent(only.spreadPercent) * 2 +
...
export async function fetchTopAssets(): Promise<TopAssetsResponse> {
