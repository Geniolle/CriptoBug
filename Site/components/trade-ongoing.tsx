  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const filtered = all.filter((x) => ONGOING_STATUSES.has(x.status))
    ...
    <div className="rounded-2xl border border-border bg-card p-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">Mostra ordens `PENDING` e `DRY_RUN`. Atualiza a cada 10s.</p>
        </div>
